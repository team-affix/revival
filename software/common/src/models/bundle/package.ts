...
export interface Package extends Bundle {
    binary: Buffer;
    payload: Buffer;
    version: string;
}
...
export function serializeDeps(deps: Map<string, string>): string {
    // Get the debuggers
...
export function deserializeDeps(deps: string): Map<string, string> {
    // Get the debuggers
...
// Compute the binary
export function computeBinary(name: string, deps: Map<string, string>, payload: Buffer): Buffer {
    // Get the debugger
    ...
    const depsSerialized = serializeDeps(deps);
...
export function computeVersion(binary: Buffer): string {
    // Get the debugger
...
export function load(filePath: string): Package {
    // Get the debugger
    ...
    // Parse the dependencies
    const deps = deserializeDeps(depsRaw);

    // Return the package
    return { name, deps, binary, payload, version };
}
