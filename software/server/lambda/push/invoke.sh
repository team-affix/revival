...
if [ "$ENTRYPOINT" == "cli" ]; then
    ...
    lambda_function_name=$(cd ../../../../infra/stacks/$ENVIRONMENT/lambda && terraform output -raw package_push_lambda_function_name)
    ...
    api_gateway_url=$(cd ../../../../infra/stacks/$ENVIRONMENT/api-gateway && terraform output -raw package_push_api_url)
