...
async function get_package_binary(registries: string[], name: string, version: string) : Promise<Buffer> {
    ...
    if (name === DEFAULT_PACKAGE_NAME) {
        const defaultPackagePath = path.join(__dirname, '..', 'default_package', DEFAULT_PACKAGE_NAME);
...
export function get_deps() : Map<string, string> {
    ...
    for (const line of depsLines) {
        ...
        if (!version)
            throw new Error(`Invalid dependency line: ${line}`);

        // If the package is already in the map, throw an error
        if (result.has(name)) {
            throw new Error(`Multiple versions of '${name}' listed in ${depsFilePath}`);
        }
